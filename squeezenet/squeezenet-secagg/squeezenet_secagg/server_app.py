...
from flwr.server import Grid, ServerApp, ServerConfig, LegacyContext
...
from flwr.server.workflow import DefaultWorkflow, SecAggWorkflow
from prometheus_client import Gauge, Counter, start_http_server

from squeezenet_secagg.task import get_weights, set_weights
...
class Log_FedAvg_Scraping(FedAvg):
    ...
    def aggregate_fit(
        ...
        torch.save(model.state_dict(), storage+f"squeezenet-tmp-r{server_round}")
        path = Path(storage+f"squeezenet-tmp-r{server_round}")
...
def main(grid: Grid, context: Context) -> None:
    ...
    global_model = SqueezeNet("1_1", 10)
