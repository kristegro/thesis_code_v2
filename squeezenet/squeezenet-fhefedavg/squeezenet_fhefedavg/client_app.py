...
from squeezenet_fhefedavg.task import load_data, set_weights, get_weights, train, test
...
def send_pds(msg: Message, context: Context):
    ...
    elif no_polynomial_encoding:
        # Hvis ikke polynomial encoding brukes korresponderer
        # kan man hente ut resultater uten videre.

        weights_sum = np.asarray([sums.GetCoefPackedValue()
                        for sums in plaintexts])
...
def weights_blocks(msg: Message, context: Context):
    ...
    if no_polynomial_encoding:
        # Uten polynomial encoding trengs kun ett slot per tall.
        num_coeffs = 1
    ...
    for p in range(pos, pos+cs):
        ...
        if (len(indices)+1)*num_coeffs <= BATCHSIZE:
            ...
            if SCHEME == "CKKS":
                ...
                if no_polynomial_encoding:
                    # Hvis man ikke bruker polynomial encdoing,
                    # rund av til nærmeste heltall og konverter til int istedenfor.
                    weights_chunk.append(int(round(weights[idx[0]][tuple(idx[1::])])))
                else:
...
def training(msg: Message, context: Context):
    ...
    current_path = Path("./storage/squeezenet-fhefedavg-training-time")
...
def evaluate(msg: Message, context: Context):
    ...
    # Lag path hvis den ikke eksisterer.
    current_path = Path("./storage/squeezenet-fhefedavg-evaluation-time")
    Path.mkdir(current_path,
               mode=0o777,
               parents=True, 
               exist_ok=True)
