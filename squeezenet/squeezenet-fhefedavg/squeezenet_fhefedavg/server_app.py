...
from squeezenet_fhefedavg.task import get_weights, set_weights
...
def main(grid: Grid, context: Context) -> None:
    ...
    for round in range(1, num_rounds+1):
        ...
        current_path = Path("./storage/squeezenet-keygen-time")
        ...
        while pos < len(struct):
            ...
            dec_stop = time.time()
            current_path = Path(storage+"squeezenet-decryption-time")
            Path.mkdir(current_path,
                    mode=0o777, 
                    parents=True, 
                    exist_ok=True)
        ...
        end_time = time.time()
        # Lag path hvis den ikke eksisterer.
        current_path = Path("./storage/squeezenet-block-time")
        Path.mkdir(current_path,
                   mode=0o777, 
                   parents=True, 
                   exist_ok=True)
        ...
        storage = os.path.abspath('storage/') + "/"
        torch.save(model.state_dict(), storage+f"squeezenet-tmp-r{round}")
        path = Path(storage+f"squeezenet-tmp-r{round}")
