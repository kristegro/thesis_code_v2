...
from prometheus import request_to_dict, dict_to_csv, runtime_to_csv, move_models
import numpy as np
...
print(parameters)
"""Sende disse inputene til stasjonær pc."""

# Bruke stasjonær sin ip.
client = mpc.Client(("192.168.68.102", 12400))
client.send(parameters)
print("Har sendt parameters til stasjonær.")
...
for test in range(parameters["num_tests"]):
    ...
    """Be stasjonær pc om metrics, sende den start- og sluttider 
    og motta metrics."""

    client = mpc.Client(("192.168.68.102", 12400))
    times = (starting_time, ending_time)
    client.send(times)
    print("Sender tider til stasjonær.")

    print("Mottar metrics tilbake.")
    desktop_metrics, desktop_ml_mean_times = client.recv()
    ...
    """Slå sammen verdier for bærbar og stasjonær.
    'result' er liste som inneholder verdier for hver container,
    så burde holde å slå de sammen med list concatination, +=."""
    for key in list(metrics_dict.keys()):
        metrics_dict[key]['data']['result'] += desktop_metrics[key]['data']['result']
    ...
    if parameters['scheme'] == "CKKS-NF" and parameters['program'] == "fhefedavg":
        ...
        runtime_to_csv(num_dirs, 
                       parameters, 
                       (runtime, desktop_ml_mean_times, laptop_ml_mean_times, noise_times, keygen_times, dec_times, block_times))
        ...
        runtime_to_csv(num_dirs, 
                       parameters, 
                       (runtime, desktop_ml_mean_times, laptop_ml_mean_times, keygen_times, dec_times, block_times))
    else:
        runtime_to_csv(num_dirs,
                       parameters, 
                       (runtime, desktop_ml_mean_times, laptop_ml_mean_times))
