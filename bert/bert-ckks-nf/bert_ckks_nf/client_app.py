...
from bert_ckks_nf.task import get_weights, load_data, set_weights, test, train
...
def create_single_ciphertext(msg: Message, context: Context):
    ...
    # First element is the trainingset size, which is not representative.
    weights = weights[1::]
...
def training(msg: Message, context: Context):
    ...
    # Deserialize crypto context. Assume ST was used for serialization.
    storage = os.path.abspath('storage/') + "/"
    ...
    current_path = Path(storage+"bert-fhefedavg-training-time")
    ...
    #     outfile.write(f"Testing set size is {len(testloader.dataset)}\n")
    #     outfile.write(f"The largest weight in the sum is {true_max}\n")
    #     outfile.write(f"The smallest weight in the sum is {true_min}\n\n")

    # Find the structure of the updates, so that decryption can be done.
...
def evaluate(msg: Message, context: Context):
    ...
    loss, accuracy, f1 = test(net, testloader, device)
    end_time = time.time()

    # Lag path hvis den ikke eksisterer.
    storage = os.path.abspath('storage/') + "/"
    current_path = Path(storage+"bert-fhefedavg-evaluation-time")
    Path.mkdir(current_path,
                mode=0o777, 
                parents=True, 
                exist_ok=True)
